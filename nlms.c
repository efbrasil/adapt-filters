...
NLMS_CTX *
nlms_init (unsigned int size, double alpha, double gamma)
{
	NLMS_CTX *n;

	n = (NLMS_CTX *) malloc (sizeof (NLMS_CTX));
...
double
nlms_iter (double x, double d, NLMS_CTX *n)
{
	double y, e, cte;
	int i;
	...
	/* adapt! */
	gsl_blas_dgemm (CblasTrans, CblasNoTrans, 1, n->state, n->state,
			0, n->result);
	cte = n->alpha * e / (n->gamma + gsl_matrix_get (n->result, 0, 0));
